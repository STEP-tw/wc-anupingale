...
describe("parse", function() {
  ...
  it("should return multiple options when multiple options are specified with having space in betn", function() {
    ...
    expectedOutput = { options: "-l-c-w", files: ["file1"] };
    assert.deepEqual(actualOutput, expectedOutput);
  });
  ...
  it("should return undefined in file names when only options are specified", function() {
    actualOutput = parse(["-l"]);
    expectedOutput = { options: "-l", files: [] };
    assert.deepEqual(actualOutput, expectedOutput);
  });
});
